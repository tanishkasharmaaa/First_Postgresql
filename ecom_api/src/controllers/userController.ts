...
export class UserController {
  ...
  static async login(req: AuthRequest, res: Response) {
    ...
    try {
      // Find user
      const result = await pool.query(
        "SELECT id, name, email, password, is_blacklisted FROM users WHERE email = $1",
        [email],
      );
  ...
  static async getProfile(req: AuthRequest, res: Response) {
    try {
      ...
      if (error instanceof AppError) throw error;
      throw new AppError(error.message || "Failed to fetch profile", 500);
    }
  }
  ...
  static async updateProfile(req: AuthRequest, res: Response) {
    const { name, phone, address, city, state, postal_code, country } =
      req.body;
    const userId = req.user?.id;
    ...
    try {
      const result = await pool.query(
        `UPDATE users SET 
         ...
         WHERE id = $8
         RETURNING id, name, email, phone, address, city, state, postal_code, country, updated_at`,
        [name, phone, address, city, state, postal_code, country, userId],
      );
  ...
  static async changePassword(req: AuthRequest, res: Response) {
    ...
    try {
      ...
      if (error instanceof AppError) throw error;
      throw new AppError(error.message || "Failed to change password", 500);
    }
  }
  ...
  static async getUserById(req: AuthRequest, res: Response) {
    ...
    try {
      const result = await pool.query(
        `SELECT id, name, email, phone, address, city, state, postal_code, 
                country, is_blacklisted, created_at FROM users WHERE id = $1`,
        [id],
      );
  ...
  static async blacklistUser(req: AuthRequest, res: Response) {
    ...
    try {
      const result = await pool.query(
        "UPDATE users SET is_blacklisted = true, updated_at = CURRENT_TIMESTAMP WHERE id = $1 RETURNING id, email",
        [id],
      );
      ...
      if (error instanceof AppError) throw error;
      throw new AppError(error.message || "Failed to blacklist user", 500);
    }
  }
}
