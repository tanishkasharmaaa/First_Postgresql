...
import { generateOrderNumber, calculateTotal } from "../utils/helpers";
...
export class OrderController {
  ...
  static async createOrder(req: AuthRequest, res: Response) {
    ...
    try {
      ...
      const cartResult = await client.query(
        `SELECT c.product_id, c.quantity, p.price, p.discount_price, p.stock_quantity
         FROM cart c
      ...
      const orderResult = await client.query(
        `INSERT INTO orders (user_id, order_number, total_amount, discount_amount, tax_amount, shipping_amount, status, payment_method, shipping_address)
      ...
      for (const item of cartResult.rows) {
        ...
        await client.query(
          `INSERT INTO order_items (order_id, product_id, quantity, unit_price, total_price)
           VALUES ($1, $2, $3, $4, $5)`,
          [orderId, item.product_id, item.quantity, price, totalPrice],
        );
  ...
  static async getOrderDetails(req: AuthRequest, res: Response) {
    ...
    try {
      const orderResult = await pool.query(
        `SELECT id, order_number, total_amount, discount_amount, tax_amount, shipping_amount, 
      ...
      const itemsResult = await pool.query(
        `SELECT oi.id, oi.product_id, p.name, p.image_url, oi.quantity, oi.unit_price, oi.total_price
         FROM order_items oi
      ...
      if (error instanceof AppError) throw error;
      throw new AppError(error.message || "Failed to fetch order", 500);
    }
  }
  ...
  static async updateOrderStatus(req: AuthRequest, res: Response) {
    ...
    try {
      ...
      const result = await pool.query(
        `UPDATE orders SET status = $1, tracking_number = $2, shipped_at = COALESCE($3, shipped_at), 
                delivered_at = COALESCE($4, delivered_at), updated_at = CURRENT_TIMESTAMP
         WHERE id = $5
         RETURNING *`,
        [status, tracking_number, shippedAt, deliveredAt, id],
      );
  ...
  static async getAllOrders(req: AuthRequest, res: Response) {
    ...
    try {
      let query =
        "SELECT id, order_number, user_id, total_amount, status, created_at FROM orders";
      const params: any[] = [];
